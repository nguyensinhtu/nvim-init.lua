...
M._cmd_run_operation = function(operation, args, arg_spec)
    ...
    a.run(function()
        ...
        notify("Running dbt operation: " .. operation).events.close(function()
...
M._create_job = function(cmd, args, arg_spec, bufname)
    ...
    local onexit_success = function(cmd, data, bufname)
        ...
        require("notify").notify(
            "Ran: " .. table.concat(cmd, " ") .. " successfully.",
            "info",
            { render = "compact", title = "dbt" }
        )
    end
        ...
        on_exit = function(j, code)
            if code >= 1 then
                ...
                vim.schedule(function()
                    onexit_success(cmd, j:result(), bufname)
                end)
            end
